...
Usage is identical to the numeric-query command.  Alternately, you can provide a timeseries ID if you
have already created a timeseries via [`create-timeseries`](#creating-timeseries).

    # convenient form
    scalyr timeseries-query '$dataset="accesslog"' --function 'bytes' --start 24h --buckets 24

    # if you've already created a timeseries via create-timeseries
...
Complete argument list:
    ...
    --end=xxx
        ...
        view. Defaults to the current time.
    --timeseries=xxx
       The id of the timeseries to query, as returned by create-timeseries; if present the
